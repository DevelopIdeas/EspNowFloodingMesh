...
function begin(portName, baudRate = 115200) {
  ...
  port.pipe(parser);
  logger.info("begin", portName, baudRate);
}
...
function ping() {
  return mutex(true).then(function (done) {
    ...
    }).timeout(5000, "Ping operation timed out");
    port.write('PING;\n')
...
function init() {
  return mutex(true).then(function (done) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      port.write('INIT;')
      return ret;
    }).timeout(5000, "Init operation timed out").then(function () {
      mutex(false);
    }).error(function (e) {
      return Promise.reject(e);
    });
...
function setBSID(bsid) {
  return mutex(true).then(function (done) {
    logger.info(`SET BSID---> 0x${bsid.toString(16)}`);
...
function setInitializationVector(iv) {
  return mutex(true).then(function (done) {
    logger.info(`InitializationVector ${iv}`);
...
function request(message, ttl = 0, timeout = 3000, wantedReplyCount = 1) {
  function reqSend() {
    var ret;
    return mutex(true).then(function () {
      logger.debug(`request ttl=${ttl}, ${message}`);
      var replyId;
      return new Promise(function (resolve, reject) {
        waitAckNack()
          .error(function () { })
          ...
          .then(function (p) {
            ...
            if (p[0] === "ACK") {
              ...
              logger.debug("Wait all Replies with replyId: " + p[1]);
            } else {
              reject("SEND FAILED");
            }
          });
